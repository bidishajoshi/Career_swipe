...
A self‑contained script to generate a realistic demo dataset for the CareerSwipe job portal.
It creates:
- 20 job seekers
- 50 companies
- 50 job postings
...
All data is emitted as SQL INSERT statements compatible with SQLite/MySQL/PostgreSQL.
The script only depends on the `faker` library (installed via pip) and the Python standard library.
"""
...
# ---------------------------------------------------------------------------
# Configuration – adjust numbers here for different dataset sizes
# ---------------------------------------------------------------------------
NUM_JOB_SEEKERS = 20
NUM_COMPANIES = 50
NUM_JOBS = 50
...
def random_date(start_days_ago=365, end_days_ago=0):
    """Return a random date between start_days_ago and end_days_ago days before today."""
...
for seeker in job_seekers:
    ...
    applied_jobs = random.sample(jobs, k=random.randint(2, 5))
    for job in applied_jobs:
        ...
        application_id += 1
    # Saved jobs – up to 3 jobs not applied to
    saved = random.sample([j for j in jobs if j not in applied_jobs], k=random.randint(0, 3))
    for job in saved:
        saved_jobs.append({
            "seeker_email": seeker["email"],
            "job_title": job["title"],
...
# Write to file
# ---------------------------------------------------------------------------
output_path = Path('c:/Users/hp/Desktop/Career_swipe/dataset_small.sql')
