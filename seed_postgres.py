...
Production‑ready seeding script for the CareerSwipe project.
It connects to the Render PostgreSQL database via the ``DATABASE_URL``
environment variable, creates all required tables (if missing) and
populates the database with realistic Nepal‑based demo data.

The script is idempotent – running it multiple times will never create
duplicate rows.

Usage:
    $ python seed_postgres.py

Make sure the ``DATABASE_URL`` environment variable points to your Render
PostgreSQL instance before running the script.
"""
...
# Keep the seed deterministic for reproducibility (optional)
# Faker.seed(12345)
...
# ---------------------------------------------------------------------------
# Models – only the subset required for the demo seeding. Use explicit
# ``UniqueConstraint`` to avoid duplicate inserts when the script is run
# more than once.
# ---------------------------------------------------------------------------
...
class User(Base):
    ...
    def set_password(self, password: str):
        # Simple SHA256 for demo – replace with real password hashing in prod
...
class Application(Base):
    ...
    status = Column(String(50), nullable=False)  # Applied, Interviewed, Shortlisted, Rejected, Hired
...
def hash_password(password: str) -> str:
    # In production replace with a proper password hasher (bcrypt, argon2, …)
...
def main():
    ...
    with Session() as session:
        # ---------------------------------------------------------------
        # Seed only when the database is empty (no users)
        # ---------------------------------------------------------------
        ...
        # ------------------- Companies -------------------
        companies = []
        for i in range(15):  # exactly 15 companies as per requirement
            # Create a user for the company admin
